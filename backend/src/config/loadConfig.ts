...
import { LLMProviderConfig, RouterConfig } from '../types/config';
...
function mergeWithEnvVars(config: Partial<RouterConfig>): RouterConfig {
  ...
  mergedConfig.providers = mergedConfig.providers.map(provider => {
...
function validateConfig(config: RouterConfig): RouterConfig {
  ...
  if (config.defaultModel) {
    const modelExists = config.providers.some(provider => 
      provider.models?.some(model => model.name === config.defaultModel)
    );
